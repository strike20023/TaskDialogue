...
from taskdialogue.benchmarks.tau2.data_model.tasks import EnvAssertion, EnvFunctionCall, InitializationData
from taskdialogue.benchmarks.tau2.environment.db import DB
from taskdialogue.benchmarks.tau2.environment.tool import Tool
from taskdialogue.benchmarks.tau2.environment.toolkit import ToolKitBase, ToolSignature, get_tool_signatures
        ...
        for tool_call, expected_response in action_responses:
            ...
            if response_content != expected_content:
                ...
                error_msg = (
                    f"Tool call:\n{tool_call}\n\n"
                    f"Returned:\n{response}\n"
                    f"(content type: {type(response_content)}, value: {response_content})\n\n"
                    f"Expected:\n{expected_response}\n"
                    f"(content type: {type(expected_content)}, value: {expected_content})"
                )
