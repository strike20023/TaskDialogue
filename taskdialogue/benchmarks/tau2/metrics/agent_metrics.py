...
from taskdialogue.benchmarks.tau2.data_model.simulation import Results
...
def display_metrics(metrics: AgentMetrics) -> None:
    logger.info(f"🏆 Average reward: {metrics.avg_reward}")
    logger.info("📈 Pass^k")
    for k, pass_hat_k in metrics.pass_hat_ks.items():
        logger.info(f"  k={k}: {pass_hat_k}")
    logger.info(f"💰 Average agent cost: {metrics.avg_agent_cost}")
