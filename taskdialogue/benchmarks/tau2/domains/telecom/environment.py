...
from taskdialogue.benchmarks.tau2.domains.telecom.data_model import LineStatus, TelecomDB
from taskdialogue.benchmarks.tau2.domains.telecom.tools import TelecomTools
from taskdialogue.benchmarks.tau2.domains.telecom.user_data_model import PaymentRequest, TelecomUserDB
from taskdialogue.benchmarks.tau2.domains.telecom.user_tools import TelecomUserTools
from taskdialogue.benchmarks.tau2.domains.telecom.utils import (
    TELECOM_DB_PATH,
...
from taskdialogue.benchmarks.tau2.utils import load_file
...
def get_environment(
    ...
    """获取 Telecom domain 的 Environment。
    
    Args:
        db: TelecomDB 实例（可选，默认从文件加载）
        user_db: TelecomUserDB 实例（可选，默认从文件加载）
        solo_mode: 是否启用 solo 模式
        policy_type: 策略类型（"manual" 或 "workflow"）
        config: 完整配置（可选，用于 profile 选择）
    
    Returns:
        TelecomEnvironment 实例
    """
    # 根据 profile 选择工具（确保 original 模式不受影响）
    if config:
        profile = config.get("tau2", {}).get("tools", {}).get("profile", "original")
        if profile == "sql":
            # SQL 模式：使用 SQL 工具（注意：telecom 不支持 SQL 模式的 user_tools）
            from taskdialogue.benchmarks.tau2.adapters.tools.registry import get_tools
            tools = get_tools(profile="sql", domain="telecom", config=config)
            user_tools = None  # SQL 模式下暂不支持 user_tools
        else:
